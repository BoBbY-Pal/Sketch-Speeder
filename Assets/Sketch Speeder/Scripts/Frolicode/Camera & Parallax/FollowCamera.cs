...
{
    [Header("Camera Target")] public GameObject target;
    ...
    void LateUpdate()
    {
        ...
        if (cam_target != null)
        {
            ...
            {
                //Move camera
                transform.position = Vector3.SmoothDamp(transform.position, target_pos, ref cur_pos, 1f / camera_speed,
        ...
        if (shake_timer > 0f)
        {
            ...
            shake_vector = new Vector3(Mathf.Cos(shake_timer * Mathf.PI * 8f) * 0.02f,
                Mathf.Sin(shake_timer * Mathf.PI * 7f) * 0.02f, 0f);
