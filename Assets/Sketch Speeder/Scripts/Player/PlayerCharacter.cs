...
namespace Sketch_Speeder.Player
{
    ...
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        [Header("Jump")] public bool can_jump = true;
        ...
        [Header("Ground Detection")] public LayerMask ground_layer;
        ...
        [Header("Fall Below Level")] public bool reset_when_fall = true;
        ...
        private void UpdateCrouch()
        {
            ...
            if (move_input.y < -0.1f && is_grounded)
            {
                ...
                capsule_coll.offset = new Vector2(coll_start_off.x,
                    coll_start_off.y - coll_start_h.y * (1f - crouch_coll_percent) / 2f);
        ...
        private bool DetectGrounded(bool detect_ceiled)
        {
            ...
            if (capsule_coll != null)
            {
                ...
                Vector2 raycast_offset = capsule_coll.offset +
                                         orientation *
                                         Mathf.Abs(capsule_coll.size.y * 0.5f - capsule_coll.size.x * 0.5f);
            ...
            for (int i = 0; i < raycastPositions.Length; i++)
            {
                ...
                {
                    if (hitBuffer[j].collider != null && hitBuffer[j].collider != capsule_coll &&
                        !hitBuffer[j].collider.isTrigger)
                    {
                        grounded = true;
                    }
                }
        ...
        public void TakeDamage()
        {
            if (!is_dead && !invulnerable)
            {
                ...
                currentLives =
                    Mathf.Clamp(currentLives, 0, maxLives); // Clamp lives between 0 and the number of heart images
                hit_timer = -1f;
        ...
        {
            if (capsule_coll != null)
                return new Vector2(Mathf.Abs(transform.localScale.x) * capsule_coll.size.x,
                    Mathf.Abs(transform.localScale.y) * capsule_coll.size.y);
